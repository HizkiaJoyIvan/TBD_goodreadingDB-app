...
const EditData = ({id}) => {
  ...
  const handleDone = async (e) => {
    ...
    try {
      ...
      await axios.put(`http://localhost:3200/api/country/${id}`, input)
...
const DeleteData = ({id}) => {
  ...
  const handleDone = async () => {
    try {
      await axios.delete(`http://localhost:3200/api/country/${id}`)
...
const AddData = () => {
  ...
  const handleDone = async (e) => {
    ...
    try {
      ...
      await axios.post(`http://localhost:3200/api/country`, input)
...
const CountryTable = () => {
  ...
  useEffect(()=> {
    const fetchData = async () => {
        try {
            const res = await axios.get(`http://localhost:3200/api/country`)
      ...
      <tbody className="bg-white divide-y divide-gray-200">
        {table.map(t => (
          <tr key={t.country_id}>
            ...
            <td className="px-6 py-4 whitespace-nowrap">{t.country_name}</td>
            <td className="px-6 py-4 whitespace-nowrap flex gap-2">
              <DeleteData id={t.country_id}/>
              <EditData id={t.country_id}/>
            </td>
          </tr>
        ))}
      </tbody>
